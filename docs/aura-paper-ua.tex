...
Фіксує версії інструментів, запускає paper vectors і attack-PoC
регресії; повні режими \texttt{test}, \texttt{formal}, \texttt{paper},
\texttt{bench}, \texttt{full} описано в artifact-документі. \\
\addlinespace
Фіксовані vectors &
\texttt{cargo test --release --test paper\_vectors} &
Стабільні вектори для HKDF, AES-256-GCM-SIV, seeded ML-KEM public key
і master-key identity derivation; байтовий handshake-KAT лишається
окремим наступним кроком через продукційну випадковість ephemeral DH та
...
Відтворюваність & Є artifact-entrypoint, fixed vectors, код, тести і proof-to-code таблиця & Додати deterministic full-handshake KATs і CI-артефакти для Tamarin/ProVerif. \\
