...
Tamarin~Prover (10/10 лем) і ProVerif (4/6 запитів; Q5/Q6 залишаються
обмеженнями інструмента для цієї DH-моделі). Реалізація мовою Rust містить близько 35{,}9~тис. рядків у
...
\begin{enumerate}
  ...
  \item[\textbf{Н2.}] \textbf{Доповнений (augmented) HKDF для неявної
    ...
    У такий спосіб досягається неявна автентифікація
    KEM-ключа без застосування додаткових цифрових підписів і без
    додаткового round-trip між сторонами. HKDF використовується не лише
    як примітив деривації, а й як механізм зв'язування ратчетного
...
\begin{enumerate}
  ...
  \item Деривує ідентифікатор сесії, ключ метаданих і ключі підтвердження:
    \begin{align}
      \mathit{sid} &\gets \HKDF(\rk_0,\,16,\,\varepsilon,\,
  ...
  \item Обчислює transcript-hash $\mathit{th}$ над канонічним бандлом і
    повідомленням ініціації, надсилає $B$ ініціюючий конверт
    $(c, \EK_A, \IK_A^x, \IK_A^{ed}, \IK_A^k, i,
    \sigma_A^{\mathit{bind}}, \mathit{mac}_I)$, де
    $\sigma_A^{\mathit{bind}}$ зв'язує Ed25519-, X25519- та KEM-ідентичності
    ініціатора,
...
INT-CTXT. Ця модель загроз мотивує використання GCM-SIV замість
...
$2^{16}-1$. Реалізація форсує новий ratchet-крок при вичерпанні
ланцюга повідомлень ($N_{\max}=10{,}000$) і викликає callback
попередження, коли залишок nonce-бюджету падає до 10\% або нижче.
...
5~с вимірювання) на Apple~M-series host, використаному для artifact-run;
артефакт фіксує точну модель, ОС, версію Rust і build profile.
...
\begin{table}[H]
\centering
\caption{Encrypt+decrypt round-trip за розміром payload.}\label{tab:bench-payload}
\small
\begin{tabular}{@{}rr@{}}
\toprule
Payload & Round-trip \\
...
Burst throughput (без ratchet, 256~Б) & $\sim$15~мкс/msg \\
\bottomrule
\end{tabular}
\end{table}
...
$259$~мкс проти близько $50$~мкс для X25519. Повідомлення в одному
напрямку уникають ratchet-роботи й вимірюються на рівні
$\sim$15~мкс/msg; alternating-профіль із 256~Б payload вимірюється на
рівні $\sim$280~мкс/msg. Повне рукостискання на benchmark-host займає
близько $1{,}1$~мс.
...
\begin{table}[H]
\centering
\caption{Чек-лист для подання, відтворення та зовнішньої перевірки.}\label{tab:submission-checklist}
\small
...
Незалежна перевірка & Є власні докази, машинні моделі та пакет для зовнішнього рецензування & Отримати зовнішнє рецензування моделі й коду; зафіксувати версії інструментів, хеші артефактів, зауваження і відповідь автора. \\
...
10~лем Tamarin, 4~доведені запити ProVerif і відображення доказових
...
становить 98~біт. Для Kyber-768 таблиця використовує оцінку
182~номінальних бітів для компонента FIPS~203 category~3; після тієї
самої втрати це дає 152~біти в AKE/HNDL режимах, де відповідний
KEM-секрет не розкривається пізніше. Реалізація мовою Rust
