...
\begin{enumerate}
  ...
  \item[\textbf{Н5.}] \textbf{Комбінований контур доказів,
    ...
    позитивні машинно-доведені результати; \emph{(г)}~Rust-тести, benchmark-и
    та FFI-межу, зіставлені з тією самою протокольною машиною станів.
\end{enumerate}
...
нового ML-KEM public key з похідним станом через HKDF info. Для групових
...
\begin{itemize}
  ...
  \item $\Encaps(\mathit{pk}) \to (c, K)$ — інкапсуляція: видає шифротекст
    $c$ та спільний секретний ключовий матеріал $K \in \{0,1\}^{\secpar}$;
...
статті з кодом не вимагало неформального пошуку по репозиторію.
...
процесом; PCS-ратчет аналізується Tamarin-моделлю ратчета та ігровою
теоремою.
...
Repo-integrated Rust-артефакт, 6 теорем, Tamarin/ProVerif, карта відповідності тверджень і коду &
...
Публічний артефакт для цього дизайн-профілю & Специфікація & Публічний SPQR repository і специфікації & Публічний дизайн, закрита реалізація & \textbf{Rust-код, тести, карта відповідності тверджень і коду} \\
