...
\begin{enumerate}
  ...
  \item[\textbf{Н5.}] \textbf{Комбінований контур доказів,
    ...
    \emph{(в)}~3 з 6 ProVerif-зобов'язань для handshake/message-path моделі;
...
\begin{itemize}
  ...
  \item $\mathsf{Corrupt}(P_i)$ — повертає всі довгострокові секретні
    ключі $P_i$ (X25519-секрет $\mathit{ik}_i$, ML-KEM-секрет
...
цієї події. Для гібридної частини розглядаємо консервативний випадок, коли до
компрометації входить також секретний KEM-ключ поточного peer-ключа, до якого
виконується перша післякомпрометаційна інкапсуляція. Тоді протокол \Aura{}
забезпечує:
\begin{enumerate}
  \item[\textbf{(а)}] \textbf{1-крокову класичну PCS}: після першого
    чесного напрямкового ratchet-переходу, виконаного після компрометації,
    ключі повідомлень, що деривуються з нового \DH{}-внеску, є
    псевдовипадковими за припущенням \GapCDH{}:
    \begin{equation}\label{eq:pcs-classical-bound}
      ...
      + \adv^{\dualPRF}_{\HKDF}(\advB_2);
    \end{equation}
  \item[\textbf{(б)}] \textbf{2-крокову гібридну PCS}: після другого
    чесного напрямкового ratchet-переходу, коли інкапсуляція вже виконується
    до KEM-ключа, згенерованого після компрометації, ключі повідомлень є
    гібридно захищеними за припущеннями \GapCDH{} та \INDCCA{} для \KEM{}:
...
ML-KEM-ключа, який входив до скомпрометованого стану. Свіжа ML-KEM-пара
...
для $q_m$ зашифрованих спроб у всіх сесіях кожний ключ повідомлення
$\mk_{e,n}$ є псевдовипадковим, а ciphertext забезпечує
...
як недоведені або негативні ProVerif-зобов'язання, а не як позитивні
твердження статті. \\
\addlinespace
Бенчмарки &
...
Repo-integrated Rust-артефакт, 6 теорем, Tamarin/ProVerif, proof-to-code таблиця &
...
Публічний артефакт для цього дизайн-профілю & Специфікація & Публічний SPQR repository і специфікації & Публічний дизайн, закрита реалізація & \textbf{Rust-код, тести, proof-to-code mapping} \\
