...
\begin{align*}
  \mathit{prk} &\gets \mathsf{Extract}(\mathit{salt}, \mathit{ikm}) = \HMAC_{\mathit{salt}}(\mathit{ikm}),\\
  \mathit{okm} &\gets \mathsf{Expand}(\mathit{prk}, \mathit{info}, L).
\end{align*}
...
\begin{enumerate}
  ...
  \item Обчислює чотири DH-секрети:
    \begin{align*}
      \mathit{DH}_1 &= \DH(\IK_A^{x,\mathit{priv}}, \SPK_B), \\
      \mathit{DH}_2 &= \DH(\EK_A^{\mathit{priv}}, \IK_B^x), \\
      \mathit{DH}_3 &= \DH(\EK_A^{\mathit{priv}}, \SPK_B), \\
...
handshake/envelope/cross-ratchet KATs і attack-PoC регресії; повні режими
...
і master-key identity derivation; deterministic handshake/envelope/cross-ratchet KATs
перевіряють байти \texttt{HandshakeInit}, \texttt{HandshakeAck}, session id
перший post-handshake envelope і перший DH+KEM ratchet envelope під
test-only entropy feature. \\
\addlinespace
...
Бенчмарки виконано за допомогою Criterion.rs (v0.5, 200~семплів,
...
Відтворюваність & Є artifact-entrypoint, fixed vectors, deterministic handshake/envelope/cross-ratchet KATs, код, тести і proof-to-code таблиця & Додати multi-epoch delayed-delivery KATs і CI-артефакти для Tamarin/ProVerif. \\
