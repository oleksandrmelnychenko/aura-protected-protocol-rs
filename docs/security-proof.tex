...
Protocol (AURA), a hybrid post-quantum secure messaging protocol combining
X25519 Diffie--Hellman key exchange with ML-KEM-768 and AES-256-GCM-SIV
...
\label{def:dh}
The proof models X25519 in the validated large prime-order subgroup of
order~$q = 2^{252} + 27742317777372353535851937790883648493$.
Curve25519 has cofactor 8; the implementation clamps scalars and rejects
small-order inputs before using a received public key. The X25519 function
...
ML-KEM public key is validated and transcript-bound by key confirmation; it is
not a separate Ed25519-signed field in the current artifact.
...
We use an $\eCK$-style AKE model~\cite{lamacchia2007} for the hybrid
post-quantum setting. The oracle set and freshness predicate below define the
exact leakage model used by the reductions.
...
\paragraph{Path K: Replace Kyber shared secret.}
...
\begin{theorem}[AKE Security in the $\eCK$-Style Model]
\label{thm:ake}
...
The initial Kyber shared secret $\mathit{ss}_{\mathsf{ky}}$ was derived by
