...
\begin{enumerate}[nosep]
  ...
  \begin{align*}
    \sid &= \HKDF(\rk_0,\; 16,\; \epsilon,\; \texttt{"Aura-SessionId"}) \\
...
Each nonce is 12 bytes: $\mathsf{prefix}(8) \concat \mathsf{counter}(2) \concat
\mathsf{msg\_index}(2)$ where $\mathsf{prefix}$ is random per epoch,
...
\begin{itemize}[nosep]
  ...
  \item If $\nu$ was evicted from the cache: eviction removes the entries
        with the \emph{smallest counters} (bytes [8..10] of the 12-byte nonce).
...
\emph{counters} (bytes [8..10] of the 12-byte nonce), not the smallest
...
In AKE/HNDL settings where the relevant KEM secret key is not later
disclosed, the Kyber component gives $182 - 30 = 152$ bits against quantum
adversaries.
