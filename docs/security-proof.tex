% ============================================================================
%  Aura Protection Protocol — Formal Security Analysis
...
\newcommand{\KEM}{\mathsf{ML\text{-}KEM\text{-}768}}
...
% ── Title ──────────────────────────────────────────────────────────────────
\title{\textbf{Formal Security Analysis of the\\Aura Protection Protocol}\\[6pt]
...
\begin{quote}
\textbf{Status of this document.} This file is retained as a historical extended
working note. It predates the narrowed assumptions, security-claim terminology,
and evidence boundaries in \texttt{aura-paper.tex}. The canonical release claims
are those in Section~6 of \texttt{aura-paper.tex}; this note is not part of the
evaluated evidence package.
\end{quote}

\begin{abstract}
We provide a rigorous game-based security analysis of the Aura Protection
Protocol (AURA), a hybrid secure-messaging protocol targeting post-quantum
confidentiality of session secrets under classical Ed25519 authentication and
combining X25519 Diffie--Hellman key exchange with ML-KEM-768 and AES-256-GCM-SIV
authenticated encryption.  AURA extends the Signal X3DH/Double Ratchet paradigm
with (i)~a hybrid combiner achieving IND-CCA2 security under the
\emph{either-or} assumption (classical Gap-CDH \emph{or} ML-KEM-768 IND-CCA2),
(ii)~per-direction hybrid ratcheting with explicit forward-secrecy and
post-compromise recovery bounds, and (iii)~two-layer AEAD with nonce-misuse
resistance.
...
(Theorem~3), post-compromise security with separate local-compromise and
two-endpoint-compromise recovery bounds (Theorem~4), message confidentiality
and integrity (Theorem~5), and replay resistance (Theorem~6).  All proofs are
...
\begin{definition}[X25519 Diffie--Hellman Abstraction]
\label{def:dh}
The proof uses an idealized prime-order X25519 abstraction of
order~$q = 2^{252} + 27742317777372353535851937790883648493$.
Curve25519 has cofactor 8; the implementation follows RFC~7748 clamping and
rejects malformed field encodings, small-order inputs, and all-zero shared
secrets where applicable before using a received public key. Cofactor and twist
subtleties are not modeled beyond this abstraction. The X25519 function
computes scalar multiplication $\DH(a, B) = [a]B$. A keypair is
$(a, A) \gets (\getsr \mathbb{Z}_q,\; [a]G)$ where $G$ is the base point
$u = 9$.
\end{definition}
...
\begin{itemize}[nosep]
  \item Under unique nonces, standard confidentiality and ciphertext integrity.
  \item Under nonce reuse, deterministic-AEAD leakage: equality may be exposed for
        repeated plaintext/AAD/nonce tuples, while ciphertext integrity is preserved.
\end{itemize}
We write $\adv^{\MRAE}_{\AEAD}(\advA)$ for the combined advantage.
\end{definition}

\subsection{Aura-Specific Notation}
...
$(\mathit{kpk}_X, \mathit{ksk}_X)$ & ML-KEM-768 keypair of party $X$ \\
...
The Aura hybrid X3DH extends the Signal X3DH~\cite{signal-x3dh} with a
ML-KEM-768 KEM component.  We formalize the protocol between initiator $I$ and
responder~$R$.
...
\[
  \mathsf{Bundle}_R = \bigl(v,\; ed_R^{\mathsf{pub}},\;
    \IK_R^{\mathsf{pub}},\; \mathit{spk\_id},\;
    \SPK_R^{\mathsf{pub}},\; \sigma_{\SPK},\;
    \{(\mathit{opk\_id}_j,\OPK_{R,j}^{\mathsf{pub}})\}_{j},\;
    \mathit{kpk}_R,\; \sigma_{\mathit{IKX}}\bigr)
\]
where $\sigma_{\mathit{IKX}}$ binds the X25519 identity key to the Ed25519 identity key
and $\sigma_{\SPK}$ signs the responder's medium-term signed pre-key. The
ML-KEM public key is shape-validated and transcript-bound by key confirmation;
it is not a separate Ed25519-signed field in the current artifact. Symbolic
authentication models that treat this field as trusted therefore use a stronger
pre-key-bundle assumption than the implementation; KEM-key substitution is
handled by the augmented-HKDF lemma and implementation tests.
...
\begin{enumerate}[nosep]
  ...
  \[
    \mathit{cs} = \HKDF\bigl(\mathsf{ikm} = 0\mathsf{xFF}^{32} \concat
      ...
      \mathsf{info} = \texttt{"Aura-X3DH"}\bigr)
  \]
  \item ML-KEM encapsulation:
  ...
  \[
    \rk_0 = \HKDF\bigl(\mathsf{ikm} = \mathit{ss}_{\mathsf{ky}},\;
      ...
      \mathsf{info} = \texttt{"Aura-Hybrid-X3DH"}\bigr)
  \]
  \item Derive session ID, metadata key, and key confirmation keys:
  \begin{align*}
    \sid &= \HKDF(\rk_0,\; 16,\; \mathit{ct}_{\mathsf{ky}},\; \texttt{"Aura-SessionId"}) \\
    \mdk_0 &= \HKDF(\rk_0,\; 32,\; \mathit{ctx}_{\mathsf{md}},\; \texttt{"Aura-MetadataKey"}) \\
    \mathit{kc}_I &= \HKDF(\rk_0,\; 32,\; \epsilon,\; \texttt{"Aura-KeyConfirm-I"}) \\
    \mathit{kc}_R &= \HKDF(\rk_0,\; 32,\; \epsilon,\; \texttt{"Aura-KeyConfirm-R"})
  \end{align*}
  where $\mathit{ctx}_{\mathsf{md}} = \mathsf{sort}(\EK_I^{\mathsf{pub}}, \SPK_R^{\mathsf{pub}}) \concat \sid$.
  \item Compute transcript hash with the key-confirmation MAC field cleared:
  \begin{multline*}
    \mathit{Init}_0 = \mathit{Init}\text{ with }\mathit{key\_confirmation\_mac}=\epsilon,\\
    \mathit{th} = \HMAC\bigl(\texttt{"Aura-Handshake-Transcript"},\;
    \mathsf{LP}(\mathsf{Bundle}_R) \concat \mathsf{LP}(\mathit{Init}_0)\bigr).
  \end{multline*}
...
$I$ receives $\mathsf{Ack}$ and verifies $\mathit{mac}_R = \mathit{mac}_R^{*}$
in constant time. On success, the implementation performs the post-handshake
chain initialization before the first message:
\[
  \mathit{dh}_{\mathit{init}}=\DH(d_{\mathit{local}},D_{\mathit{remote}}),
  \qquad
  \mathit{ikm}_{\mathit{init}}=\mathit{dh}_{\mathit{init}}\concat\mathit{ss}_{\mathsf{ky}},
\]
\[
  T_{\mathit{init}}=\HKDF(\mathit{ikm}_{\mathit{init}},64,\rk_0,\texttt{"Aura-Hybrid-Ratchet"}),
  \qquad
  \rk_1=T_{\mathit{init}}[0{:}32],
\]
\[
  (\ck_{\mathit{send}},\ck_{\mathit{recv}})
    =\HKDF(\rk_1,64,\epsilon,\texttt{"Aura-ChainInit"}).
\]
The established message-layer state is therefore
$(\rk_1,\ck_{\mathit{send}},\ck_{\mathit{recv}},\mdk_0,\sid)$; $\mdk_0$
remains the initial metadata key until the first ratchet.
...
\begin{align*}
  \mk &= \HKDF(\ck,\; 32,\; \epsilon,\; \texttt{"Aura-Msg"}) \\
  \ck' &= \HKDF(\ck,\; 32,\; \epsilon,\; \texttt{"Aura-Chain"})
\end{align*}

\subsubsection{\texorpdfstring{Hybrid Ratchet Step $R$}{Hybrid Ratchet Step R}}
...
$N_{\max}$ messages.  This makes the implementation trigger explicit instead
of leaving recovery to an implicit message-pattern assumption.
...
\begin{enumerate}[nosep]
  ...
  \item Generate fresh ML-KEM keypair: $(\mathit{kpk}_{e+1}, \mathit{ksk}_{e+1}) \gets \KEM.\KeyGen()$.
  \item Combine: $\mathit{ikm} = \mathit{dh}_{e+1} \concat \mathit{ss}_{e+1}$.
  \item \textbf{Augmented HKDF} (binds ML-KEM public key into derivation):
  \[
    (\rk_{e+1},\; \ck_{e+1},\; \mdk_{e+1}^{\mathsf{send}}) =
    \HKDF\bigl(\mathit{ikm},\; 96,\;
      \rk_e,\;
      \texttt{"Aura-Hybrid-Ratchet"} \concat \mathit{kpk}_{e+1}\bigr)
  \]
...
Payload nonces are 12 bytes:
$\mathsf{prefix}(8) \concat \mathsf{counter}(2) \concat
\mathsf{msg\_index}(2)$, where $\mathsf{prefix}$ is sampled by the CSPRNG
when the send nonce generator is created or reset at a ratchet boundary,
$\mathsf{counter}$ is monotonically increasing, and $\mathsf{msg\_index}$
is the chain position. Metadata encryption uses an independent 12-byte
header nonce.
...
We use a modified X3DH leakage model inspired by $\eCK$~\cite{lamacchia2007}
for the hybrid post-quantum setting. The oracle set and freshness predicate
below define the exact leakage model used by the reductions.

\subsection{Parties and Sessions}

There are at most $N$ parties $P_1, \ldots, P_N$.  Each party $P_i$ has a
long-term X25519 identity keypair $\IK_i$, a long-term Ed25519 signing key,
a long-term ML-KEM-768 keypair $(\mathit{kpk}_i, \mathit{ksk}_i)$,
rotating signed pre-keys $\SPK_i$, and one-time pre-keys.
...
\begin{itemize}[nosep]
  ...
  \item $\mathsf{Corrupt}(P_i)$: Return all long-term secret keys of $P_i$
        ($ik_i$, Ed25519 signing key, $\mathit{ksk}_i$). Models full
        long-term key compromise.
  \item $\mathsf{RevealSPK}(P_i,\mathit{spk\_id})$: Return the private scalar
        for a selected signed pre-key. Models medium-term pre-key compromise
        separately from identity-key and KEM secret-key compromise.
  \item $\mathsf{RevealOPK}(P_i,\mathit{opk\_id})$: Return the private scalar
        for an unused one-time pre-key. Consumed OPKs are modeled as deleted.
  \item $\mathsf{RevealState}(\pi_i^s)$: Return the full session state of
        $\pi_i^s$ (root key, chain keys, metadata keys, DH private key,
        KEM secret key, nonce state). Models state compromise at a point in time.
...
\begin{enumerate}[nosep]
  \item $\advA$ queried $\mathsf{RevealSessionKey}$ or
        $\mathsf{RevealState}$ on $\pi_i^s$ or $\pi_j^t$ before
        $\mathsf{Test}$.
  ...
  \item $\advA$ queried $\mathsf{RevealSPK}(P_j,\mathit{spk\_id})$ for the
        responder signed pre-key used in the tested session before
        $\mathsf{Test}$ \textbf{and} no OPK was used and deleted.
  \item $\advA$ queried $\mathsf{RevealOPK}(P_j,\mathit{opk\_id})$ for the
        OPK used in the tested session before it was consumed.
\end{enumerate}

\subsection{AKE Security}

\begin{definition}[$\eCK$-Style AKE Security]
\label{def:ake}
Protocol $\Pi$ is secure in the model above if for all $\ppt$ adversaries $\advA$:
\[
  \adv^{\eCK}_\Pi(\advA) \;=\;
...
compromise, for sessions whose responder signed-pre-key private scalar remains
undisclosed or whose consumed OPK private scalar has already been deleted.
\end{definition}
...
% ============================================================================
\section{Theorem 1: Hybrid Root-Key Pseudorandomness}
\label{sec:thm1}
% ============================================================================

\begin{theorem}[Hybrid Root-Key Pseudorandomness]
\label{thm:combiner}
If $\KEM$ is $\INDCCA$ secure \textbf{or} $\DH$ satisfies $\GapCDH$ in the
$\ROM$, then the Aura hybrid combiner
\[
  \rk_0 = \HKDF(\mathsf{ikm} = \mathit{ss}_{\mathsf{ky}},\;
    \mathsf{salt} = \mathit{cs},\;
    \mathsf{info} = \texttt{"Aura-Hybrid-X3DH"})
\]
outputs a pseudorandom root key.  Concretely, for any $\ppt$ adversary $\advA$:
\[
  ...
  + \frac{q_H}{2^{256}}
\]
where $q_H$ is the number of random oracle queries and the factor 4 accounts
for guessing which of the four X3DH Diffie--Hellman computations is embedded.
\end{theorem}

\begin{proof}
We use two alternative reductions and then take the tighter bound.

\paragraph{Game 0.}
The real key-indistinguishability game for the hybrid combiner.  The challenger generates
classical DH keypairs and ML-KEM keypairs, computes the classical shared secret
...
\texttt{"Aura-Hybrid-X3DH"})$.  With probability $1/2$, the challenger
replaces $\rk_0$ with a uniform random value.  $\advA$'s advantage is
$\adv^{\mathsf{rk}}_{\mathsf{Hybrid}}(\advA)$.

\paragraph{Path K: Replace ML-KEM shared secret.}
...
\[
  |\Pr[S_K] - \Pr[S_0]| \;\leq\; \adv^{\INDCCA}_{\KEM}(\advB_1).
\]
In $S_K$, the HKDF input keying material is uniformly random.

\paragraph{Path D: Replace classical DH output.}
Alternatively, if the classical component is secure, we construct
...
$\mathit{ss}_{\mathsf{ky}}$ may remain real, then $\advB_2$ can solve
Gap-CDH on at least one of the four DH pairs:
\[
  |\Pr[S_D] - \Pr[S_0]| \;\leq\; 4 \cdot \adv^{\GapCDH}_{\DH}(\advB_2).
\]
...
In $S_D$, the HKDF salt is uniformly random.

\paragraph{Apply dual-PRF.}
Choose the better of Path~K and Path~D.  In the resulting game,
\emph{at least one} of the HKDF inputs ($\mathit{ss}_{\mathsf{ky}}$ as IKM,
or $\mathit{cs}$ as salt) is uniformly random.  By the dual-PRF property of
$\HKDF$ (\cref{def:dualprf}), the output $\rk_0$ is computationally
indistinguishable from random:
\[
  |\Pr[S_3] - \Pr[S_*]| \;\leq\; \adv^{\dualPRF}_{\HKDF}(\advB_3),
\]
where $S_* \in \{S_K,S_D\}$ is the selected path.

In Game~3, $\rk_0$ is uniformly random regardless of $b$, so
$\advA$ has zero advantage.  Combining:
\[
  \adv^{\mathsf{rk}}_{\mathsf{Hybrid}}(\advA)
  \;\leq\; \min\!\left\{\adv^{\INDCCA}_{\KEM}(\advB_1),
  4 \cdot \adv^{\GapCDH}_{\DH}(\advB_2)\right\}
...
The implementation defines domain labels in the constants module.
HKDF domain separation applies only to labels used as HKDF \texttt{info}
values. The handshake-transcript, identity-binding, init-identity-binding, and
PQ-hybrid labels are domain separators for HMAC, hashing, signatures, or helper
paths rather than HKDF labels. For HKDF calls, distinct
\texttt{info} values separate derived keys by purpose. In the ROM, distinct
\texttt{info} values produce distinct oracle inputs; in the standard model, the
separation follows from the PRF property of HKDF-Expand.
\end{lemma}

\begin{proof}
In the HKDF construction, the \texttt{info} string is an input to HMAC-SHA256 in
the expand phase: $T_i = \HMAC(K_{\mathsf{prk}}, T_{i-1}\concat
\mathsf{info}\concat i)$. Since the protocol uses different \texttt{info}
values for root keys, chain keys, message keys, metadata keys, session IDs, and
key-confirmation keys, the HMAC inputs are distinct. The outputs are therefore
independent under the PRF assumption on HMAC-SHA256. \qedhere
\end{proof}
...
primitives (ML-KEM-768 or X25519) remains unbroken.  This is the standard
hedging guarantee for a hybrid KEM without assuming both components are
simultaneously secure, and matches the design goal of hedging against quantum
attacks on classical DH while retaining classical security as a fallback if
lattice-based assumptions are weakened.
\end{remark}
...
\begin{theorem}[AKE Security in the Modified X3DH Leakage Model]
\label{thm:ake}
The Aura hybrid X3DH satisfies \cref{def:ake}
under the $\GapCDH$, $\INDCCA$ (ML-KEM-768), $\dualPRF$ (HKDF-SHA256), and
$\ROM$ assumptions.  For any $\ppt$ adversary $\advA$ making at most $q_s$
session queries:
\begin{multline*}
  \adv^{\eCK}_{\mathsf{AURA}}(\advA) \;\leq\;
  q_s \cdot \bigl(
...
The challenger runs the AURA protocol honestly.  The adversary interacts via
oracles from \cref{def:oracles} and makes a $\mathsf{Test}$ query on a fresh
session.  $\advA$'s advantage is $\adv^{\eCK}_{\mathsf{AURA}}(\advA)$.
...
the four DH computations. By the modified freshness condition, at least one
DH pair $(a, B)$ has \emph{either} $a$ uncompromised (no $\mathsf{RevealState}$)
\emph{or} the long-term key uncompromised (no $\mathsf{Corrupt}$). The
freshness predicate also excludes responder signed-pre-key disclosure for
tested no-OPK sessions.
...
\paragraph{Game 3: Replace ML-KEM shared secret.}
By the modified freshness condition, if the adversary has corrupted $R$'s
long-term KEM key $\mathit{ksk}_R$, then $R$'s ephemeral state (specifically
...
\begin{enumerate}[nosep]
  ...
  \item Find a transcript collision $\mathit{th} = \mathit{th}'$ for different
        ...
        \texttt{"Aura-Handshake-Transcript"}, which is computationally
...
\[
  \SHA\bigl(\texttt{"Aura-Identity-Binding"} \concat
...
\label{thm:fs}
The Aura protocol achieves forward secrecy (\cref{def:fs}).  Specifically,
for any $\ppt$ adversary $\advA$ who obtains long-term identity and KEM keys
$(\IK_I, \IK_R, \mathit{ksk}_I, \mathit{ksk}_R)$ at time $t$, and for sessions
whose responder signed-pre-key private scalar remains undisclosed or whose
consumed OPK private scalar has already been deleted:
\[
  \adv^{\mathsf{FS}}_{\mathsf{AURA}}(\advA)
  \;\leq\;
  \adv^{\GapCDH}_{\DH}(\advB_1)
  + \adv^{\dualPRF}_{\HKDF}(\advB_2)
\]
for sessions whose handshake completed before time~$t$. If both the responder
SPK private scalar and KEM secret key are later disclosed and no deleted OPK
contributed, this classical FS theorem does not apply.
\end{theorem}
...
The adversary receives long-term identity and KEM keys at time $t$ and must
distinguish a past session key from random.

\paragraph{Game 1: Surviving DH term.}
For a target session that completed before~$t$, the initiator ephemeral key
$ek_I$ was generated, used, and erased before the compromise. The term
$\mathit{dh}_2=\DH(ek_I,\IK_R^{\mathsf{pub}})$ is not counted after $ik_R$
disclosure, because DH is commutative and the adversary can recompute it from
$ik_R$ and the transcript value $\EK_I^{\mathsf{pub}}$. Security comes from a
term whose peer private scalar is still unavailable:
\begin{itemize}[nosep]
  \item $\mathit{dh}_3 = \DH(ek_I, \SPK_R^{\mathsf{pub}})$ if the responder
        signed-pre-key private scalar is not revealed; or
  \item $\mathit{dh}_4 = \DH(ek_I, \OPK_R^{\mathsf{pub}})$ if a one-time
        pre-key was used and the consumed private scalar has been deleted.
\end{itemize}

We embed a Gap-CDH challenge in the surviving DH pair. $\advA$ cannot compute
the shared secret without solving Gap-CDH:
\[
  |\Pr[S_1] - \Pr[S_0]| \;\leq\; \adv^{\GapCDH}_{\DH}(\advB_1).
\]

\paragraph{Game 2: Dual-PRF application.}
The surviving DH contribution makes the classical shared secret computationally
random, and that value is used as the HKDF salt. By the dual-PRF property:
\[
  |\Pr[S_2] - \Pr[S_1]| \;\leq\; \adv^{\dualPRF}_{\HKDF}(\advB_2).
\]

In Game~2, the session's root key is random.  The adversary has zero
advantage.

\paragraph{Scope of the KEM contribution.}
The initial ML-KEM shared secret $\mathit{ss}_{\mathsf{ky}}$ was derived by
encapsulating to $\mathit{kpk}_R$.  Under the FS oracle above, $\advA$ later
learns $\mathit{ksk}_R$ and has the recorded ciphertext
$\mathit{ct}_{\mathsf{ky}}$, so this initial KEM component is not counted as
forward-secret.  It provides harvest-now-decrypt-later protection only if the
corresponding KEM secret key is not later disclosed.  Post-compromise
post-quantum recovery from fresh ratchet KEM keys is covered separately by
\cref{thm:pcs}. \qedhere
\end{proof}

\begin{lemma}[Ephemeral Key Erasure]
\label{lem:eph-erasure}
The implementation erases temporary key material immediately after use:
\begin{itemize}[nosep]
  \item temporary copies of $ek_I$ are wiped after DH computation; the X25519
        key that becomes current ratchet state remains in guarded memory until
        the next ratchet replaces it.
...
\end{itemize}
This erasure pattern reduces the window in which post-handshake memory
inspection can recover temporary secrets, while active ratchet private keys are
treated as live session state until replaced. The argument assumes
\texttt{zeroize} provides effective memory erasure through volatile writes and
compiler barriers.
\end{lemma}
...
$\mathsf{RevealState}$) and then stops learning post-compromise secrets, then:
\begin{enumerate}[(a)]
  \item \textbf{Local classical PCS (1-step)}: If the compromise is local to
        the sender endpoint, or the peer DH private scalar used by the next
        directed DH remains undisclosed, messages at epoch $\geq e{+}1$ are
        secure under $\GapCDH$:
        \[
          \adv^{\mathsf{PCS\text{-}1}}_{\mathsf{AURA}}(\advA)
          \;\leq\; \adv^{\GapCDH}_{\DH}(\advB_1) +
          \adv^{\dualPRF}_{\HKDF}(\advB_2).
        \]
  \item \textbf{Two-endpoint hybrid recovery}: If both endpoints' epoch-$e$
        DH and KEM ratchet secrets are exposed, no first-step PCS is claimed.
        Messages at epoch $\geq e{+}2$, after a directed step uses
        post-compromise peer DH and KEM public keys, are secure under
        $\GapCDH$ \textbf{and} $\KEM$-$\INDCCA$:
        \[
          \adv^{\mathsf{PCS\text{-}2}}_{\mathsf{AURA}}(\advA)
          \;\leq\; \adv^{\GapCDH}_{\DH}(\advB_1) +
          \adv^{\INDCCA}_{\KEM}(\advB_3) +
          2 \cdot \adv^{\dualPRF}_{\HKDF}(\advB_2).
        \]
\end{enumerate}
Under one-endpoint compromise, the first ratchet step may already include an
uncompromised peer KEM contribution; the conservative two-endpoint clause does
not rely on that.
\end{theorem}

\begin{proof}
The key subtlety is the \emph{asymmetric recovery time} between local and
two-endpoint compromise. The first post-compromise directed step still uses the
...
from the sender's new public key.

\paragraph{Setup.} At epoch $e$, $\advA$ obtains:
\begin{itemize}[nosep]
  \item Root key $\rk_e$, chain keys $\ck_e$, metadata keys $\mdk_e$.
  \item DH private key $d_e$ and KEM secret key $\mathit{ksk}_e$.
  \item Peer's public keys: $D_{\mathsf{peer}}$ (DH) and $\mathit{kpk}_e^{\mathsf{peer}}$ (KEM).
  \item For the conservative two-endpoint recovery claim, the peer's epoch-$e$
        DH private scalar and KEM secret key corresponding to
        $D_{\mathsf{peer}}$ and $\mathit{kpk}_e^{\mathsf{peer}}$.
\end{itemize}
...
\paragraph{Game 1: Epoch $e{+}1$ --- fresh local DH, possibly compromised peer KEM key.}
...
\]
In the local-compromise case, the adversary knows $D_{\mathsf{peer}}$ but not
the corresponding peer private scalar, so computing $\mathit{dh}_{e+1}$
requires solving Gap-CDH:
\[
  |\Pr[S_1] - \Pr[S_0]| \;\leq\; \adv^{\GapCDH}_{\DH}(\advB_1).
\]

\textbf{However}, the KEM encapsulation at epoch $e{+}1$ uses
$\mathit{kpk}_e^{\mathsf{peer}}$ (the \emph{peer's} KEM public key from
epoch~$e$, which the adversary obtained during compromise).  The adversary
knows the corresponding $\mathit{ksk}_e^{\mathsf{peer}}$ in the conservative
two-endpoint compromise model, so $\mathit{ss}_{e+1}$ is known to the
adversary.
...
If the peer DH private scalar is also compromised, this first DH value is
computable and no first-step PCS is claimed. Otherwise $\rk_{e+1}$ is
pseudorandom. This proves part~(a): 1-step classical PCS for the local or
peer-DH-fresh compromise scope.

\paragraph{Game 2: Epoch $e{+}2$ --- fresh DH and fresh KEM key.}
At epoch $e{+}1$, the honest party also generated fresh DH and ML-KEM keypairs
and advertised their public components in the ratchet header. The adversary
does \emph{not} know the post-compromise private scalars or
$\mathit{ksk}_{e+1}$.
...
The DH component at epoch $e{+}2$ also uses post-compromise DH material rather
than the peer's exposed epoch-$e$ DH scalar. Applying the dual-PRF property once more:
\[
  |\Pr[S_3] - \Pr[S_2]| \;\leq\; \adv^{\dualPRF}_{\HKDF}(\advB_2).
\]

$\rk_{e+2}$ is pseudorandom with both components contributing. This proves
part~(b): two-endpoint recovery after the next directed step that uses
post-compromise peer DH and KEM material. \qedhere
\end{proof}

\begin{lemma}[Per-Direction Ratcheting]
\label{lem:per-dir}
AURA's per-direction ratcheting (\path|send_ratchet_pending| flag set after
every received message) ensures that \emph{every direction change} triggers a
fresh hybrid ratchet step.  This differs from Signal's DH-key-change trigger in
the following operational sense:
\begin{enumerate}[nosep]
  \item In Signal, if Alice sends 100 messages then Bob replies, only Bob's
        reply carries the next ratchet public key.  In AURA, Bob's reply
        triggers a ratchet and Alice's next message also triggers a directed
        hybrid ratchet.
  \item This makes the PCS recovery point explicit: after local compromise,
        classical PCS starts at the next honest ratchet step; after
        two-endpoint compromise, hybrid recovery needs a directed step that
        uses post-compromise peer DH and KEM material.
\end{enumerate}
\end{lemma}
...
\[
  \mathsf{info} = \texttt{"Aura-Hybrid-Ratchet"} \concat \mathit{kpk}_{e+1}.
\]
This binds the new ML-KEM public key $\mathit{kpk}_{e+1}$ into the key
derivation.  If an adversary substitutes a different KEM public key
...
binding check for the KEM public key without per-step signatures; it is not a
separate identity-authentication mechanism.
\end{lemma}
...
\[
  \rk'_{e+1} = \HKDF(\mathit{ikm},\; \rk_e,\;
    \texttt{"Aura-Hybrid-Ratchet"} \concat \mathit{kpk}').
\]
The sender computed:
\[
  \rk_{e+1} = \HKDF(\mathit{ikm},\; \rk_e,\;
    \texttt{"Aura-Hybrid-Ratchet"} \concat \mathit{kpk}_{e+1}).
\]
...
\label{thm:conf-int}
For fresh partnered sessions under non-adversary-established root keys, and
...
For any $\ppt$ adversary $\advA$ over $q_m$ total encrypted message attempts
across all sessions:
\begin{multline*}
  \adv^{\mathsf{Conf}}_{\mathsf{AURA}}(\advA)
  \;\leq\;
  \adv^{\eCK}_{\mathsf{AURA}}(\advB_0) +
  q_m \cdot \adv^{\PRF}_{\mathsf{chain\text{-}KDF}}(\advB_1) \\
  {} + q_m \cdot \adv^{\MRAE}_{\AEAD}(\advB_2) +
  \frac{q_m^2}{2^{96}}
\end{multline*}
where the last term bounds random metadata header-nonce collisions under the
same metadata key; payload nonce collision is zero within a chain by
construction.
\end{theorem}
...
root key plus fresh DH/KEM material, so by induction and \cref{thm:pcs}:
\[
  |\Pr[S_1] - \Pr[S_0]| \;\leq\; \adv^{\eCK}_{\mathsf{AURA}}(\advB_0).
\]
...
$(\mk, \ck') = (\HKDF(\ck, \texttt{"Aura-Msg"}),\;
\HKDF(\ck, \texttt{"Aura-Chain"}))$
...
The domain separation between \texttt{"Aura-Msg"} and
\texttt{"Aura-Chain"} ensures the message key and next chain key are
...
This provides:
\begin{itemize}[nosep]
  \item Confidentiality under unique nonces; under nonce reuse GCM-SIV degrades
        to deterministic encryption and leaks equality for repeated
        plaintext/AAD/nonce tuples.
...
Payload nonces are 96 bits: $\mathsf{prefix}(64) \concat
\mathsf{counter}(16) \concat \mathsf{msg\_index}(16)$ (all in bits).
Within a single chain, payload nonces are unique by construction (counter is
monotonic and the chain is rotated before either 16-bit field is exhausted).
Across sessions, nonce collision probability is bounded by the birthday bound
on the 64-bit random prefix:
\[
  \Pr[\text{cross-session nonce collision}] \;\leq\; \frac{q_s^2 \cdot q_m^2}{2^{64}}.
\]
...
\]
The $q_m^2 / 2^{96}$ term in the theorem is instead charged to the random
metadata \texttt{header\_nonce} under a fixed metadata key. A repeated metadata
nonce would have GCM-SIV's deterministic-AEAD equality leakage, not a full GCM
catastrophe.
...
\begin{itemize}[nosep]
  \item $\mdk_e$ is a separate 32-byte segment of the ratchet HKDF output.
  \item $\mk_{e,n}$ is derived later from $\ck_e$ by the chain KDF under the
        \texttt{"Aura-Msg"} domain label.
\end{itemize}
For ratchet epochs, $\ck_e$ and $\mdk_e$ are disjoint segments of one jointly
pseudorandom HKDF output rather than outputs of distinct HKDF-info strings.
Under the HKDF PRF assumption and the one-way chain derivation, compromise of
one layer key does not reveal the other layer key.
...
\begin{itemize}[nosep]
  ...
  \item Under nonce reuse: degrades to $\mathsf{DAE}$ (deterministic
        authenticated encryption) --- repeated key/nonce/AAD/plaintext tuples
        expose equality patterns. $\INTCTXT$ is preserved.
\end{itemize}
Since AURA uses monotonic counters for nonces within each chain, nonce reuse
...
\label{thm:replay}
The combination of unique payload nonces, consumed message keys, and the
bounded seen-nonce cache provides replay resistance under continuous receiver
state and no accepted sealed-state rollback. Exact replay acceptance is zero;
for modified replays:
\[
  \adv^{\mathsf{Replay}}_{\mathsf{AURA}}(\advA) \;\leq\;
...
\begin{itemize}[nosep]
  ...
  \item If $\nu$ was evicted from the cache: the consumed message key is no
        longer derivable from the current chain state. The stale ciphertext
        cannot be accepted without rolling the receiver state back.
\end{itemize}

Thus exact replay acceptance is $0$ under the theorem's state-continuity
assumption. If an application accepts an older sealed state after a crash or
restore, the violated premise is rollback freshness, not AEAD security.
...
Combining: $\adv^{\mathsf{Replay}}_{\mathsf{AURA}}(\advA) \leq
\adv^{\INTCTXT}_{\AEAD}(\advB)$. \qedhere
\end{proof}

\begin{remark}[Nonce Pruning Correctness]
The nonce cache pruning evicts the oldest observed nonce entries from the
bounded insertion-order window. This preserves recent entries for replay
detection without relying on ad hoc parsing of the nonce counter.
\end{remark}
...
\begin{center}
\textbf{Concrete security levels for AURA components and composite bounds.}

\renewcommand{\arraystretch}{1.3}
\small
\begin{tabular}{@{}p{0.40\textwidth}p{0.32\textwidth}p{0.15\textwidth}@{}}
\toprule
...
HKDF-SHA256 / Dual-PRF & RFC 5869 & PRF, 256-bit output \\
...
Forward secrecy (\cref{thm:fs}), Gap-CDH with erased DH ephemeral & --- & 128 \\
...
adversaries. ML-KEM-768 is treated as a NIST category-3 KEM; concrete
Core-SVP bit estimates are estimator-dependent and are not protocol
constants.
...
AURA uses \emph{per-direction} ratcheting: every received message sets
$\mathsf{send\_ratchet\_pending} = \mathsf{true}$, triggering a full
hybrid ratchet (X25519 + ML-KEM-768) on the next outgoing message.  This
provides:

\begin{enumerate}[nosep]
  \item \textbf{Per-direction forward secrecy}: Each direction change produces
        fresh key material, limiting the window of vulnerability for a
        compromised chain key.
  \item \textbf{Explicit PCS recovery point}: After local state compromise,
        classical PCS starts at the next honest ratchet step. After
        two-endpoint state compromise, recovery needs a directed step that
        uses peer DH and KEM material generated after the compromise.
  \item \textbf{Post-quantum contribution per ratchet step}: Each ratchet
        contributes both classical and PQ key material.
\end{enumerate}

The cost is increased computational overhead and bandwidth overhead
($+1184 + 1088 = 2272$ bytes per ratchet step for ML-KEM public key +
...
\begin{itemize}[nosep]
  \item \textbf{Signal (classical only)}: 1-step PCS recovery via fresh DH.
        ...
        recovers security.
  \item \textbf{AURA (hybrid)}: local endpoint compromise admits 1-step
        \emph{classical} PCS. If both endpoints' current DH and KEM ratchet
        secrets were exposed, no first-step PCS is claimed; hybrid recovery
        starts only when a directed step uses peer DH and KEM material generated
        after compromise.
\end{itemize}

This statement is scoped to Aura's directed KEM-public-key schedule. Other
SCKA-style schedules can trade bandwidth, acknowledgements, and state retention
for different recovery latency.

\subsection{Identity Binding}

AURA computes the identity binding hash as
\[
  \SHA\bigl(\texttt{"Aura-Identity-Binding"} \concat
  \mathsf{sort}(\mathit{ed}_{I}, \mathit{ed}_{R}) \concat
  \mathsf{sort}(\mathit{x}_{I}, \mathit{x}_{R})\bigr)
\]
and embeds this in every AEAD additional data. Signal-style protocols obtain
identity binding through authenticated transcripts, signed pre-keys, and
associated data; Aura adds the following implementation-level binding:
\begin{enumerate}[nosep]
  \item Identity keys are explicitly included in the authenticated data.
  \item The sorted canonicalization gives a stable unordered peer-set binding;
        role separation is provided by transcript/key-confirmation domains and
        send/receive chain assignment.
  \item SHA-256 domain separation (\texttt{"Aura-Identity-Binding"})
...
This appendix gives a game-based security analysis of the Aura Protection
Protocol, establishing six security properties through constructive reductions.
The hybrid combiner achieves an either-or guarantee (secure if \emph{at least
one} of X25519 or ML-KEM-768 is secure), the ratchet gives 1-step classical PCS
for the local-compromise case and delayed hybrid recovery for full
two-endpoint state compromise under the stated model, and the two-layer AEAD
with GCM-SIV provides nonce-misuse-resistant confidentiality and integrity.
...
and ML-KEM shared secret as IKM; (2)~the explicit characterization of 1-step
local-compromise vs.\ two-endpoint PCS recovery in \cref{thm:pcs}; and (3)~the
augmented HKDF info string binding that provides an implicit state-binding
check for the KEM public key without per-step signatures.
...
``AES-GCM-SIV: nonce misuse-\allowbreak resistant authenticated encryption,''
...
B.~LaMacchia, K.~Lauter,\allowbreak{} and A.~Mityagin,
