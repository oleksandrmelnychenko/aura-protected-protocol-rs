...
long-term X25519 identity keypair $\IK_i$, a long-term ML-KEM-768 keypair
...
inspection can recover ephemeral secrets, assuming \texttt{zeroize} provides
effective memory erasure through volatile writes and compiler barriers.
\end{lemma}
...
\begin{enumerate}[(a)]
  ...
  \item \textbf{Hybrid PCS (2-step)}: Messages at epoch $\geq e{+}2$ are
        ...
        \]
\end{enumerate}
The hybrid 2-step statement is stated for the conservative two-endpoint
compromise model, where both endpoints' epoch-$e$ session states, including
the KEM secret key targeted by the first post-compromise encapsulation, are
exposed.  Under one-endpoint compromise, the first ratchet step may already
...
\begin{itemize}[nosep]
  ...
  \item For the conservative two-endpoint hybrid-PCS claim, the peer's
        epoch-$e$ KEM secret key corresponding to
        $\mathit{kpk}_e^{\mathsf{peer}}$.
\end{itemize}
...
\paragraph{Game 1: Epoch $e{+}1$ --- fresh DH, compromised KEM key.}
...
At epoch $e{+}1$, the honest party also generated a fresh ML-KEM keypair
...
Now \emph{both} DH and KEM components at epoch $e{+}2$ are fresh.  Applying
...
\begin{enumerate}[nosep]
  \item In Signal, if Alice sends 100 messages then Bob replies, only Bob's
        ...
        hybrid ratchet.
  \item This makes the PCS recovery point explicit: classical PCS starts at
        the next honest ratchet step, and full hybrid PCS needs the second
        directed hybrid step.
\end{enumerate}
\end{lemma}
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Explicit PCS recovery point}: After state compromise,
        classical PCS starts at the next honest ratchet step; full hybrid PCS
        needs the second directed hybrid step because the peer must first
        advertise a fresh KEM public key.
  \item \textbf{Post-quantum contribution per ratchet step}: Each ratchet
        contributes both classical and PQ key material.
\end{enumerate}

The cost is increased computational overhead ($\sim$1 ms per ML-KEM
...
\begin{itemize}[nosep]
  \item \textbf{Signal (classical only)}: 1-step PCS recovery via fresh DH.
        ...
        recovers security.
  \item \textbf{AURA (hybrid)}: 1-step \emph{classical} PCS, 2-step
        \emph{hybrid} PCS in the directed KEM-public-key schedule. At the first
        post-compromise directed step, the new DH component is secure (fresh
        keypair) but the KEM encapsulation uses the \emph{old} peer KEM public
        key from the compromised state. Full hybrid security is restored on the
        next directed step that uses the new KEM public key generated after
        compromise.
\end{itemize}
...
and 2-step hybrid PCS under the stated compromise model, and the two-layer AEAD
