...
The 15 distinct HKDF info strings used in AURA%
\footnote{%
\texttt{Aura-X3DH}, \texttt{Aura-Hybrid-X3DH},
\texttt{Aura-Hybrid-Ratchet}, \texttt{Aura-DH-Ratchet},
\texttt{Aura-ChainInit}, \texttt{Aura-Chain},
\texttt{Aura-Msg}, \texttt{Aura-SessionId},
\texttt{Aura-MetadataKey}, \texttt{Aura-OPAQUE-Root},
\texttt{Aura-State-HMAC}, \texttt{Aura-KeyConfirm-I},
\texttt{Aura-KeyConfirm-R}, \texttt{Aura-Handshake-Transcript},
\texttt{Aura-Identity-Binding}.%
}
...
primitives (Kyber-768 or X25519) remains unbroken.  This is the standard
...
\label{thm:ake}
The Aura hybrid X3DH satisfies $\eCK$-AKE security (\cref{def:ake})
...
The classical shared secret is computationally random and is used as the HKDF
salt.  By the dual-PRF property:
\[
...
Each nonce is 96 bits: $\mathsf{prefix}(64) \concat
...
birthday bound on the 64-bit random prefix:
\[
...
\begin{itemize}[nosep]
  ...
  \item If $\nu$ was evicted from the cache: eviction removes the oldest
        observed nonce entries. Since the replayed message is outside the
        receiver's retained window and the ratchet epoch has advanced, the
        chain key and message key have
...
one} of X25519 or Kyber-768 is secure), the ratchet gives 1-step classical PCS
