...
\begin{align}
  ...
  \rk_1 &= \HKDF(\mathit{hybrid\_ikm},\, 32,\, \rk_0,\, \texttt{"Aura-Hybrid-Ratchet"})[0{:}32], \\
  (\ck_{\mathit{send}},\, \ck_{\mathit{recv}}) &= \HKDF(\rk_1,\, 64,\, \varepsilon,\, \texttt{"Aura-ChainInit"}).
\end{align}
...
handshake transcript, the first post-handshake envelope, and the first
DH+KEM-ratchet envelope under a test-only deterministic entropy feature. The
remaining reproducibility work is mechanical rather than conceptual: add
multi-epoch delayed-delivery KATs without weakening production entropy, publish
one benchmark script with pinned tool versions, and archive Tamarin/ProVerif
outputs for a tagged release.
