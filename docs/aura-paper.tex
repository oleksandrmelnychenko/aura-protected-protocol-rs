...
% ─── Title ───────────────────────────────────────────────────
\title{\textbf{Aura: A Hybrid Post-Quantum Secure Messaging Protocol\\with Per-Ratchet KEM Rotation and Symbolic Models}}
...
% ─────────────────────────────────────────────────────────────
\begin{abstract}
Aura is a hybrid post-quantum messaging protocol built around X25519 and ML-KEM-768. It uses post-quantum material in the initial X3DH-style handshake and again when the message direction changes: each directed ratchet step derives a new root key from fresh X25519 and KEM inputs. In the conservative compromise model analyzed here, this gives classical post-compromise recovery after one honest ratchet step and hybrid recovery after two directed steps.
...
The security argument consists of six game-based theorems under standard assumptions (Gap-CDH, ML-KEM IND-CCA2, dual-PRF, and MRAE), together with symbolic models: 10 Tamarin lemmas verify, and 3 of 6 ProVerif obligations discharge for the handshake/message-path model. The remaining ProVerif obligations are not counted as machine-checked results: injective authentication does not terminate in the default four-DH model, the broad message-integrity query admits adversary-originated sessions, and the raw KEM-secret phase query is false after KEM secret-key disclosure. The implementation contains approximately 35.9k lines in \texttt{src/}, 30.1k lines of tests, and 1.1k lines of benchmarks. The full repository test matrix contains 750 default scenarios and 889 scenarios with \texttt{ffi} enabled.
\end{abstract}
...
The proof models X25519 in the validated large prime-order subgroup of order
$q = 2^{252} + 27742317777372353535851937790883648493$. Curve25519 itself
has cofactor 8; the implementation clamps scalars and rejects small-order
inputs before using a received public key. The X25519 function~\cite{rfc7748}
computes scalar multiplication $\DH(a, B) = [a]B$. A keypair is $(a, A)$
where $A = [a]G$ and $G$ is the base point $u = 9$.
\end{definition}
...
The metadata layer ($\mdk_e$) and payload layer ($\mk_{e,n}$) use keys derived from different HKDF computations with distinct info strings. Given only $\mk_{e,n}$ and not the root or metadata key material, the adversary obtains no information about $\mdk_e$ under the HKDF assumptions.
\end{lemma}
...
There are two reduction paths. In the KEM path, we replace the Kyber shared
...
\begin{theorem}[$\eCK$-Style AKE Security]\label{thm:ake}
...
pseudorandom. The Kyber term is intentionally not counted in this FS bound:
after later disclosure of $\mathit{ksk}_R$, a recorded KEM ciphertext can be
decapsulated. Kyber's contribution in this setting is HNDL protection without
...
compromise model, messages at epoch $\geq e+2$ are secure:
...
Lemma~4 (\texttt{forward\_secrecy\_hybrid}) is the hybrid secrecy property in the Tamarin handshake abstraction: if the adversary learns the session key $k$, then either (a) a party was compromised \emph{before} the session (active impersonation), or (b) \emph{full} compromise (including Kyber SK) occurred. Classical-only compromise after the session does not break secrecy in that model.
...
\begin{enumerate}
  \item \textbf{Guarded memory allocation.} Long-lived secret keys (DH private keys, Kyber secret keys) are stored in \texttt{SecureMemoryHandle} objects. On Linux/Unix targets that permit page locking, this uses \texttt{mlock} and, on Linux, \texttt{MADV\_DONTDUMP}; on platforms that cannot lock pages, the implementation falls back to heap storage with automatic zeroing via \texttt{zeroize} on drop.
...
PQ-PCS through SCKA epochs; progress depends on Braid chunks/codewords and participant message-sending behavior &
...
Fresh PQ material binding mechanism & Handshake KEM only & Public SCKA/Braid state specifies KEM-key updates & Public PQ3 material describes signed in-band PQ rekeying fields & \textbf{KEM public key in HKDF info} \\
Separate per-epoch envelope-metadata AEAD & No comparable PQXDH layer & Signal Double Ratchet has a header-encryption variant; no comparable per-epoch envelope-metadata AEAD is claimed for this SPQR baseline & No independent per-epoch envelope key is claimed in the public design & \textbf{Per-epoch metadata key} \\
...
The 2-step hybrid PCS recovery follows from the directed KEM-public-key ratchet used in Aura: at epoch $e+1$ after compromise, the fresh DH provides classical security, but the Kyber encapsulation uses the compromised peer public key. Full hybrid security is restored at epoch $e+2$ when the peer's fresh Kyber public key (generated at $e+1$) is used.

Within this KEM-public-key ratchet pattern, we conjecture that 1-step hybrid PCS for \emph{both} parties simultaneously would require either: (a) both parties to independently generate and transmit fresh KEM public keys in the same protocol message, or (b) a post-quantum interactive key exchange primitive (analogous to DH but based on lattice assumptions).
...
fixed traffic-profile comparisons. The remaining reproducibility work is external rather than
...
Aura isolates a concrete design point for hybrid post-quantum messaging: KEM material is refreshed as part of the continuing ratchet state, not only during the initial X3DH-style handshake. The protocol combines per-ratchet KEM-key rotation, explicit KEM-key binding, per-epoch metadata encryption, and nonce-misuse-resistant AEAD.

The evidence is split into the layers a reviewer can check: 6 game-based theorems, 10 Tamarin lemmas, 3 discharged ProVerif obligations for the analyzed two-party protocol, a proof-to-code map, and a Rust implementation. The current implementation measures approximately $1.1$\,ms for the full handshake and approximately $15\,\mu$s/msg for burst messages; the test matrix contains 750 default scenarios and 889 scenarios with \texttt{ffi} enabled, including property-based and concurrent stress tests.
