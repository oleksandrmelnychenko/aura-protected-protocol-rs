...
\medskip
\noindent\textbf{Our contributions.} We present \textsc{Aura}, a hybrid post-quantum secure messaging protocol that addresses all four challenges:
...
We present six theorems establishing the security of the Aura protocol. Full proofs are available in the companion technical report~\cite{aura-security-proof}; here we provide theorem statements and proof sketches.
...
\[
  \adv^{\eCK}_{\mathrm{AURA}}(\advA) \leq q_s \cdot \left(4 \cdot \adv^{\GapCDH}_{\DH}(\advB_1) + \adv^{\INDCCA}_{\KEM}(\advB_2) + \adv^{\dualPRF}_{\HKDF}(\advB_3) + 2 \cdot \adv^{\PRF}_{\HMAC}(\advB_4)\right) + \frac{q_s^2}{2^{128}} + \frac{q_H}{2^{256}}.
\]
\end{theorem}
...
The Aura protocol is implemented in Rust (edition 2021, MSRV 1.70), comprising approximately 5{,}400 lines of library code organized into the following modules:
...
\toprule
Feature & Signal X3DH & Signal PQXDH & \textbf{Aura} \\
\midrule
