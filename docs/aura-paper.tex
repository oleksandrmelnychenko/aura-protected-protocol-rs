...
\medskip
\noindent\textbf{Contributions.} The paper makes five technical contributions:

\begin{enumerate}
  \item \textbf{Per-direction hybrid ratcheting.} Every direction change triggers a full X25519~+~Kyber-768 key exchange and feeds both secrets into the next root key. The resulting PCS bound separates one-step classical recovery from two-step hybrid recovery (\cref{thm:pcs}).

  \item \textbf{Augmented HKDF key derivation.} Each fresh Kyber public key is included in the HKDF info parameter during ratchet key derivation. This binds the advertised KEM key to the transcript without adding a per-ratchet signature (\cref{lem:augmented-hkdf}).

  \item \textbf{Two-layer AEAD with metadata encryption.} Envelope metadata and payload bytes are encrypted under separate keys. The metadata key rotates by epoch, and both layers use AES-256-GCM-SIV for nonce-misuse resistance (\cref{lem:two-layer}).

  \item \textbf{Proof and model files.} The analysis includes 6 theorems, 8 supporting lemmas, 10 verified Tamarin lemmas, and 4 verified ProVerif queries. The proof obligations are mapped to the Rust modules and tests that enforce the corresponding invariants.

  \item \textbf{Rust implementation.} The repository includes secure memory handling via \texttt{mlock} and \texttt{zeroize}, property-based and concurrent tests, benchmarks, and a C~FFI boundary.
\end{enumerate}
