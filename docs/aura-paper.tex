...
The security argument consists of six game-based theorems under standard assumptions (Gap-CDH, ML-KEM IND-CCA2, dual-PRF, and MRAE), together with symbolic models: 10 Tamarin lemmas verify, and 4 of 6 ProVerif queries verify. The two remaining ProVerif queries are not discharged because ProVerif overapproximates Diffie--Hellman equational reasoning in this model: Q6 is supported by Tamarin-style secrecy/PCS models and game-based proofs, while Q5 is supported by the game-based message-security proof and implementation tests. The implementation contains approximately 35.9k lines in \texttt{src/}, 30.1k lines of tests, and 1.1k lines of benchmarks. The test matrix contains 750 default scenarios and 889 scenarios with \texttt{ffi} enabled.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{N5: proof-to-code artifact chain.} The repository records explicit links among 6 theorems, 8 supporting lemmas, 10 verified Tamarin lemmas, 4 verified ProVerif queries, Rust tests, benchmarks, FFI boundaries, and the protocol state machine.
\end{enumerate}
...
$\sigma_I^{\mathit{bind}} \gets \textsf{Ed25519.Sign}(ik_I^{\mathit{ed}},\, \mathit{ed}_I^{\mathit{pub}} \concat \IK_I^{\mathit{pub}} \concat kpk_I)$\;
...
Each 12-byte nonce consists of an 8-byte epoch prefix and two 2-byte counters:
...
\end{equation}
The prefix is derived for the epoch, the counter is monotonically increasing,
and the message index provides per-chain addressability. The implementation
...
\begin{itemize}
  ...
  \item Nonces embed a monotonic counter (bytes $[8{:}10]$); when the cache is full, entries with the smallest counter values are evicted first.
...
\begin{equation}\label{eq:combiner-bound}
  \adv^{\INDCCA}_{\mathrm{Hybrid}}(\advA)
  \leq
  \min\!\left\{
...
The dominant security loss arises from the session-guessing factor $q_s$ in \cref{thm:ake}. For $q_s \leq 2^{30}$: effective security is $128 - 30 = 98$ bits classical. In AKE/HNDL settings where the relevant KEM secret key is not later disclosed, the Kyber component gives $182 - 30 = 152$ bits against quantum adversaries. The FS and PCS bounds in \cref{thm:fs,thm:pcs} do not add a separate session-guessing factor.
...
The game-based proofs are accompanied by machine-checked symbolic models in two tools: Tamarin Prover~\cite{meier2013} for trace-based reasoning and ProVerif~\cite{blanchet2016} for Horn-clause analysis. Across these tool-specific models, 14 of 16 queried properties discharge; the two remaining ProVerif queries are reported as unproved obligations.
...
\begin{itemize}
  \item Only 2 of the 4 X3DH DH operations ($\mathit{dh}_1$ and $\mathit{dh}_2$) are modeled to avoid Tamarin's source saturation with $\geq 4$ DH operations. This abstraction is intended to be conservative: omitting DH contributions gives the adversary fewer secrets to distinguish and weakens the modeled protocol.
...
The ProVerif model (\texttt{aura.pv}) is more detailed than the Tamarin ratchet abstraction: it models all four X3DH DH operations, Ed25519 signature verification, the full chain KDF, and augmented HKDF info strings. It uses one public Dolev--Yao channel with 10 distinct symbolic types and supports unbounded session replication.
...
Q5 and Q6 fail due to limitations of ProVerif's handling of the Diffie--Hellman equational theory: ProVerif uses Horn clause resolution that overapproximates the DH adversary's capabilities. The resulting traces have not been mapped to implementation attacks, so they are treated as unproved ProVerif obligations. Q6 is supported by the Tamarin-style secrecy/PCS models and the game-based proofs; Q5 is supported by the game-based message-security proof and implementation tests.

\subsection{Complementarity of Tools}

The two tools are complementary: Tamarin provides precise DH reasoning but requires abstractions to avoid non-termination; ProVerif handles unbounded sessions with all four DH operations but overapproximates DH-dependent properties. The reported result is therefore 10/10 Tamarin lemmas and 4/6 ProVerif queries, with Q5/Q6 kept outside the machine-checked claim.
...
\begin{itemize}
  \item \textbf{Self-testing.} Every keypair generation is followed by an immediate encapsulate/decapsulate round-trip with constant-time shared secret comparison. This detects KEM correctness failures, including gross RNG or library faults that cause encapsulation and decapsulation to disagree.
...
\paragraph{Analysis.}
In these measurements, a hybrid ratchet step costs about $259\,\mu$s, versus about $50\,\mu$s for X25519 alone. Same-direction burst messages avoid ratchet work and measure about $15\,\mu$s/msg; alternating 256-byte messages measure about $280\,\mu$s/msg. The full handshake measures about $1.1$\,ms on the benchmark host.
...
Two of six ProVerif queries (Q5: message integrity, Q6: forward secrecy) fail due to ProVerif's overapproximation of the DH equational theory. These are known limitations documented in the ProVerif literature. Q6 is supported by the Tamarin-style secrecy/PCS models and the game-based proofs; Q5 is supported by the game-based message-security proof and implementation tests.
...
The evidence is split into the layers a reviewer can check: 6 game-based theorems, 10 Tamarin lemmas, 4 proven ProVerif queries for the analyzed two-party protocol, a proof-to-code map, and a Rust implementation. The current implementation measures approximately $1.1$\,ms for the full handshake and approximately $15\,\mu$s/msg for burst messages; the test matrix contains 750 default scenarios and 889 scenarios with \texttt{ffi} enabled, including property-based and concurrent stress tests.
