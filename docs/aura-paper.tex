...
Q5 and Q6 fail due to known limitations of ProVerif's handling of the Diffie--Hellman equational theory: ProVerif uses Horn clause resolution that overapproximates the DH adversary's capabilities, producing spurious attacks that do not correspond to real protocol vulnerabilities. Q6 is covered by the Tamarin-style secrecy/PCS models and the game-based proofs; Q5 is covered by the game-based message-security proof and implementation tests.

\subsection{Complementarity of Tools}

The two tools are complementary: Tamarin provides precise DH reasoning but requires abstractions to avoid non-termination; ProVerif handles unbounded sessions with all four DH operations but overapproximates DH-dependent properties. Combined, 14 out of 16 total queries are machine-verified. The 2 unproven ProVerif queries are treated as documented tool limitations rather than machine-checked results.
...
The implementation is validated by the current test matrix: 750 scenarios in the default configuration and 889 scenarios with \texttt{ffi} enabled.
