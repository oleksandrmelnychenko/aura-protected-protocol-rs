...
The construction binds each new KEM public key into the HKDF transcript and separates payload encryption from metadata encryption by using an independent per-epoch metadata key. Relative to Signal \SPQR{}/Triple Ratchet and Apple \PQThree{}, the claim is limited to an open Rust artifact that exposes KEM-key binding, metadata-key rotation, and proof-to-code links in one inspectable implementation.
...
$\mathit{th} \gets \HMAC(\texttt{"Aura-Handshake-Transcript"},\, \mathit{len}(\mathit{Bundle}_R) \concat \mathit{Bundle}_R \concat \mathit{len}(\mathit{Init}_{\mathit{mac}=0}) \concat \mathit{Init}_{\mathit{mac}=0})$\;
...
uses an independent 12-byte \texttt{header\_nonce}. The implementation
forces a ratchet at the message-chain limit ($N_{\max}=10{,}000$), before
...
The 15 distinct HKDF info strings used in Aura separate key derivations by purpose. In the $\ROM$, distinct info strings correspond to distinct oracle inputs; in the standard model, separation follows from the $\PRF$ property of HKDF-Expand.
\end{lemma}
...
KEM secret keys. The initial KEM ciphertext gives harvest-now-decrypt-later
protection only while the corresponding KEM secret key is not later disclosed.
\end{theorem}

\begin{proof}[Proof sketch]
In Game~1, the DH computation with the erased ephemeral key $\mathit{ek}_I$
is hard under $\GapCDH$. The resulting classical shared secret is used as the
HKDF salt, so the $\dualPRF$ property makes the root key pseudorandom. The
Kyber term is intentionally not counted in this FS bound: after later
disclosure of $\mathit{ksk}_R$, a recorded KEM ciphertext can be decapsulated.
Kyber's contribution in this setting is HNDL protection without later KEM
secret-key disclosure, and PCS after fresh ratchet KEM keys appear.
\end{proof}
...
\begin{proof}[Proof sketch]
Exact replays are caught by the nonce cache ($W$ entries, evicting the oldest observed nonce entries). If a nonce was evicted, the stale message key is no longer derivable. Probability of cache miss on a valid nonce: $W/2^{96}$ per attempt. Modified replays (altered ciphertext) require forging an AEAD tag, reducing to $\INTCTXT$.
\end{proof}
...
Lemma~1 (\texttt{pcs\_sender\_compromise}) captures the PCS property under the terminal one-step ratchet abstraction: even with the sender's full state compromised (adversary knows $\rk_e$), a fresh hybrid ratchet step restores secrecy unless the receiver is also compromised.

\subsection{ProVerif}\label{sec:proverif}

The active ProVerif model (\texttt{aura.pv}) covers the handshake/message path. It models all four X3DH DH operations, Ed25519 signature verification, the initial chain KDF, and augmented HKDF info strings. It uses one public Dolev--Yao channel with 10 distinct symbolic types and supports unbounded session replication. Ratchet PCS is analyzed by the Tamarin ratchet model and the game-based theorem, not by an active ProVerif ratchet process.
...
Q6 & Forward secrecy (phase) & Phase-1 compromise does not reveal phase-0 KEM secret & $\times$ \\
\bottomrule
\end{tabularx}
\end{table}
...
The Kyber-768 integration has three implementation points that affect the protocol artifact:
...
Benchmarks are single-host Criterion.rs measurements from \texttt{cargo bench --bench protocol\_bench} with 200 samples and 5-second measurement time. Values are hardware-dependent implementation-cost snapshots, not security assumptions. The artifact records OS/CPU details, Rust versions, build-profile context, \texttt{Cargo.lock} hash, stdout logs, and Criterion result files. Results are summarized in \cref{tab:bench-core,tab:bench-payload,tab:bench-overhead}. The public benchmark harness also contains a fixed 128-message, 256-byte \texttt{pq\_traffic\_profiles} group for comparing handshake-only PQ, sparse chain-boundary PQ, periodic chain-boundary PQ, and per-reply PQ rekeying under the same measurement harness.
...
Binding of fresh KEM public key to transcript & Handshake KEM only & Public SCKA/Braid state specifies KEM-key updates & Public PQ3 material describes in-band PQ rekeying & \textbf{KEM public key in HKDF info} \\
Separate rotating header protection & No & No separate envelope-metadata layer is claimed in the public spec & No independent per-epoch envelope key is claimed in the public design & \textbf{Per-epoch metadata key} \\
Open implementation & Specification & Specification & Public design, closed implementation & \textbf{Rust code, tests, proof-to-code map} \\
